...
# Replace validateStatus
validateStatus_regex = r'    private fun validateStatus\(url: String\): Boolean \{.*?\n    \}'
...
content = re.sub(validateStatus_regex, new_validateStatus, content, flags=re.DOTALL)

# Replace findUrlOrNull
findUrl_regex = r'    private fun findUrlOrNull\(\s*format: PlayerResponse\.StreamingData\.Format,\s*videoId: String\s*\): String\? \{.*?\n    \}'
...
content = re.sub(findUrl_regex, new_findUrl, content, flags=re.DOTALL)
